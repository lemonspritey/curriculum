...
<form action='answer.php' method='post'>
  ...
  <?php foreach ($options[0] as $option) { ?>
    <input type="radio" name="response[0]" value="<?php echo $option ?>" />
    <span> <?php echo $option; ?></span>
  <?php } ?>

  <h2>②Webページを作成するための言語は？</h2>
  <?php foreach ($options[1] as $option) { ?>
    <input type="radio" name="response[1]" value="<?php echo $option ?>" />
    <span> <?php echo $option; ?> </span>
  <?php } ?>

  <h2>③MySQLで情報を取得するためのコマンドは？</h2>
  <?php foreach ($options[2] as $option) { ?>
    <input type="radio" name="response[2]" value="<?php echo $option ?>" />
    <span> <?php echo $option; ?></span>
  <?php } ?>
  <br>
